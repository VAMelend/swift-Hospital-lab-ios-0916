    ...
    override func spec() {
        ...
        describe("Reprimand protocol") {
            it("Should provide default implementation of disciplineStudent(_:) function. See instructions for details.") {
                ...
                let messageForParentSevere = fakeJim.callParentDeliveringMessage(_scale: .severe)
                let messageForParentLight = fakeJim.callParentDeliveringMessage(_scale: .light)
                let messageForParentMedium = fakeJim.callParentDeliveringMessage(_scale: .medium)
